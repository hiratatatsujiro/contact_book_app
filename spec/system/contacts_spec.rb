...
RSpec.describe '連絡詳細', type: :system do
  ...
  it 'ログインしたユーザーはツイート詳細ページに遷移してコメント投稿欄が表示される' do
    ...
    visit contact_path(@contact)
    # ページに情報が存在する
    expect(page).to have_content("#{@contact.condition.name}")
    expect(page).to have_content("#{@contact.reason}")
    expect(page).to have_content("#{@contact.pool_marathon.name}")
    expect(page).to have_content("#{@contact.contact}")
  end
